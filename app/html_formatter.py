    ...
    for track in tracks:
        ...
        linked_title = f"""
            <div class="track-title">
                <a
                    href="{format_youtube_search(track[:2])}"
                    target="_blank"
        ...
        artist_marked_w_explicit = f"""<div class="artist">{track[1]}</div>"""
        if track[2]:
            artist_marked_w_explicit = f"""
            <div class="explicit">{track[1]} (explicit)</div>"""
        table_list.append(
